...
# Main function: BPEMA
# Assist function: data_rm, press_count
# Draw boxplot for the output data from BPEMA: plotBPEMA


#### data_rm: function for removing missing or duplicate data ####
...
#### press_count: count the presses around each survey for a single observation ####
...
#### BPEMA: the main data manipulation function, together with the visualization plots ####
BPEMA = function(BP, EMA, time.win, returnPlot = 1, Dup_time = 5){
  # Input:
  ...
  #   Dup_time: time window for duplicate observations, used for removing duplication observations 
  ...
  ema = data_rm(ema, dup_time = Dup_time) %>%
    filter(ID %in% BP$ID)
  ...
  #            time_day = time_day, run.time = run.time)
  bp = data_rm(bp, dup_time = Dup_time)
...
#### plot_BPEMA: the visualization plots for data generated by the BPEMA function ####
